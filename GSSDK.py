...
class GSRequest():
        ...
        def UrlEncode(self, value): 
                if not value:
                        ...
                        S = quote_plus(str(value))
                        return S.replace("+","%20")
        
        def traceField(self, name, value):
                if value:
                        self._traceLog.append(str(name)+ "=" + str(value))
...
class GSResponse():
        ...
        def traceField(self, name, value):
                if value:
                        self.traceLog.append(str(name)+ "=" + str(value))
        ...
        def __init__(self, method, responseText=None, params=None, errorCode=None, errorMessage=None, traceLog=None):
                ...
                if responseText:
                        ...
                        self.rawData = responseText
                        
                        if sys.version_info[0] >= 3:
                                self.rawData = self.rawData.encode('utf-8')
                                
                        
                        if(responseText.lstrip().find('{') != -1):
                                self.data = self.data = jsonparse(responseText, encoding='utf-8')
...
class SigUtils():
        ...
        def calcSignature(baseString,key):
                ...
                rawHmac = hmac.new(b64decode(key), encodedBase, sha1).digest()
